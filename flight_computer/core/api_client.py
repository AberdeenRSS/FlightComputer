...
class ApiClient:
    ...
    async def authenticate(self):
        ...
        # use None for the account parameter.
        try:
            result = await self.request_with_error_handling_and_retry(lambda client: client.post('/auth/authorization_code_flow', timeout=30, data=json.dumps({ 'token': self.auth_code })), 3, False) # type: ignore
        except Exception as e:
            self.logger.exception(f'Authentication failed: {e.args}')
            raise
